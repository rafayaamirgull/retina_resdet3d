...
from data.datasets import (
    ...
    get_filtered_frames,
    filter_labels,
    read_ava_annotations,
)
from data.datasets import (
    get_frame_level_annos_ucf24,
    get_filtered_tubes_ucf24,
    read_labelmap,
)
...
def compute_iou(box, cls_gt_boxes):
    ...
    for m in range(cls_gt_boxes.shape[0]):
        ...
        iw = np.maximum(xmax - xmin, 0.0)
        ih = np.maximum(ymax - ymin, 0.0)
        if iw > 0 and ih > 0:
            intsc = iw * ih
        else:
            intsc = 0.0
        union = (
            (gtbox[2] - gtbox[0]) * (gtbox[3] - gtbox[1])
                ...
                for k in sorted_ids:  # start from best scoring detection of cls to end
                    ...
                    if cls_gt_boxes.shape[0] > 0:
                        ...
                        if iou[maxid] >= iou_thresh:
                            ispositive = (
                                True  # if yes then this is ture positive detection
                            )
...
def get_gt_tubes(final_annots, subset, label_type, dataset):
    ...
    for videoname in final_annots["db"]:
        if dataset == "road":
            cond = is_part_of_subsets(
                final_annots["db"][videoname]["split_ids"], [subset]
            )
        else:
        ...
        if cond:
            ...
            if dataset == "road":
                ...
                tubes[videoname] = get_filtered_tubes_ucf24(
                    final_annots["db"][videoname]["annotations"]
                )
    ...
    for video, video_tubes in tubes.items():
        for tube in video_tubes:
            if tube["label_id"] == cl_id:
                ...
                class_tubes.append(
                    [video, tube]
                )  # make_det_tube(scores, boxes, frames, label_id)])
    return class_tubes
...
def compute_class_ap(class_dets, class_gts, match_func, iou_thresh, metric_type=None):
    ...
    fn = max(1, sum([len(class_gts[iid]) for iid in class_gts]))  # false negatives
    num_postives = fn
    ...
    for count, det_id in enumerate(indexs):
        ...
        if len(class_gts[iid]) > 0:
            if metric_type is None:
                ious = np.asarray([match_func(det, gt) for gt in class_gts[iid]])
            else:
                ious = np.asarray(
                    [match_func(det, gt, metric_type) for gt in class_gts[iid]]
                )
            # print(ious)
...
def evaluate_tubes(
    ...
    for _, label_type in enumerate(label_types):
        ...
        for cl_id, class_name in enumerate(classes):
            ...
            class_ap, num_postives, count, recall = compute_class_ap(
                class_dets,
                class_gts,
                get_tube_3Diou,
                iou_thresh,
...
def get_gt_frames_ucf24(final_annots, label_type):
    ...
    for videoname in final_annots["db"]:
        if videoname not in trainvideos:
            ...
            fframe_level_annos, _ = get_frame_level_annos_ucf24(
                database[videoname]["annotations"], numf, num_classes
            )
            for frame_id, frame in enumerate(fframe_level_annos):
                frame_name = "{:05d}".format(int(frame_id + 1))
                all_boxes = []
                label = (
                    0 if label_type == "action_ness" else database[videoname]["label"]
                )
...
def get_gt_frames_ava(final_annots, label_type):
    ...
    assert label_type in [
        "action_ness",
        "actions",
    ], "only valid for action classes not for actionness but TODO: should be easy to incorprate just add to eval_framewise_ego_actions_ucf24 as preds are same but gt in this format {}".format(
        label_type
    )
...
def evaluate_frames(anno_file, det_file, subset, iou_thresh=0.5, dataset="road"):
    ...
    if dataset == "road":
        ...
        raise Exception(
            "Define data type prpperly follwong is not in the list ::: " + dataset
        )
    ...
    for nlt, label_type in enumerate(label_types):
        if label_type in ["av_actions", "frame_actions"]:
            mAP, ap_all, ap_strs = eval_framewise_ego_actions(
                final_annots, detections[label_type], subset, dataset
            )
            ...
            for cl_id, class_name in enumerate(classes):
                ...
                if dataset == "ava" and label_type != "action_ness":
                    class_gts = get_gt_class_frames(
                        gt_frames, label_map[class_name]["org_id"]
                    )
                else:
                ...
                class_dets = get_det_class_frames(
                    detections[label_type], cl_id, frame_ids, dataset
                )
                t3 = time.perf_counter()

                class_ap, num_postives, count, recall = compute_class_ap(
                    class_dets, class_gts, compute_iou_dict, iou_thresh
                )
