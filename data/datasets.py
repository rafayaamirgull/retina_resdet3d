...
def get_frame_level_annos_ucf24(annotations, numf, num_classes, counts=None):
    frame_level_annos = [
        {"labeled": True, "ego_label": 0, "boxes": [], "labels": []}
        for _ in range(numf)
    ]
    add_one = 1
    # if num_classes == 24:
    ...
    for tubeid, tube in enumerate(annotations):
        ...
        for frame_index, frame_num in enumerate(
            np.arange(tube["sf"], tube["ef"], 1)
        ):  # start of the tube to end frame of the tube
            label = tube["label"]
            # assert action_id == label, 'Tube label and video label should be same'
            box, counts = get_box(
                tube["boxes"][frame_index, :].copy(), counts
            )  # get the box as an array
...
def get_frame_level_annos_ava(
    annotations, numf, num_classes, class_ids_map, counts=None, split="val"
):
    frame_level_annos = [
        {"labeled": False, "ego_label": -1, "boxes": [], "labels": []}
        for _ in range(numf)
    ]
    ...
    for ts in timestamps:
        ...
        if ts in annotations:
            ...
            for anno in annotations[ts]:
                ...
                assert (
                    80 >= anno[2] >= 1
                ), "label should be between 1 and 80 but it is {} ".format(anno[2])
...
def get_filtered_tubes_ucf24(annotations):
    ...
    for tubeid, tube in enumerate(annotations):
        ...
        for frame_index, frame_num in enumerate(np.arange(tube["sf"], tube["ef"], 1)):
            ...
            boxes.append(box)
            count += 1
        assert count == tube["boxes"].shape[0], "numb: {} count ={}".format(
            tube["boxes"].shape[0], count
        )
                ...
                for key in frame_annos:
                    ...
                    if label_key == "agent_ness":
                        ...
                        filtered_ids = filter_labels(
                            anno[label_key + "_ids"], all_labels, labels
                        )
...
class VideoDataset(tutils.data.Dataset):
    ...
    def _make_lists_ava(self):
        ...
        self.anno_file = os.path.join(
            self.anno_root, "ava_{}.csv".format(self.SUBSETS[0])
        )
        ...
        for vid, video_name in enumerate(annotations):
            ...
            numf = 27029
            frame_level_annos, counts, keyframes, skip_count = (
                get_frame_level_annos_ava(
                    annotations[video_name],
                    numf,
                    self.num_classes,
                    class_ids_map,
                    counts=counts,
                    split=self.SUBSETS[0],
                )
            )
            ...
            start_frames = [
                kf - self.MAX_SEQ_STEP * self.SEQ_LEN // 2 for kf in keyframes
            ]
        ...
        logger.info(
            "Labeled frames {:d}/{:d}".format(total_labeled_frame, total_num_frames)
        )
        # pdb.set_trace()
        ptrstr = "\n"
        ...
        ptrstr += "Labeled frames {:d}/{:d}".format(
            total_labeled_frame, total_num_frames
        )
        self.childs = {}
    ...
    def _make_lists_ucf24(self):
        ...
        for videoname in sorted(database.keys()):
            ...
            frame_level_annos, counts = get_frame_level_annos_ucf24(
                annotations, numf, self.num_classes, counts
            )
            ...
            for frame_index in range(numf):  # frame_level_annos:
                if len(frame_level_annos[frame_index]["labels"]) > 0:
                    frames_with_boxes += 1
                frame_level_annos[frame_index]["labels"] = np.asarray(
                    frame_level_annos[frame_index]["labels"], dtype=np.float32
                )
                frame_level_annos[frame_index]["boxes"] = np.asarray(
                    frame_level_annos[frame_index]["boxes"], dtype=np.float32
                )
            ...
            start_frames = [
                f
                for f in range(
                    numf - self.MIN_SEQ_STEP * self.SEQ_LEN, -1, -self.skip_step
                )
            ]
            ...
            for frame_num in start_frames:
                step_list = [
                    ...
                    if numf - s * self.SEQ_LEN >= frame_num
                ]
                shuffle(step_list)
        ...
        for videoname in sorted(database.keys()):
            ...
            for frame_num in sorted(
                ...
                if frame_id in frames.keys() and frames[frame_id]["annotated"] > 0:
                    ...
                    frame_level_annos[frame_index]["ego_label"] = frames[frame_id][
                        "av_action_ids"
                    ][0]
    ...
    def __getitem__(self, index):
        ...
        labels = []
        ego_labels = []
        mask = np.zeros(self.SEQ_LEN, dtype=np.int8)
        indexs = []
        for i in range(self.SEQ_LEN):
            indexs.append(frame_num)
            if self.DATASET != "ava":
                img_name = self._imgpath + "/{:s}/{:05d}.jpg".format(
                    videoname, frame_num
                )
                img_name = self._imgpath + "/{:s}/{:05d}.jpg".format(
                    videoname, frame_num
                ...
                img_name = self._imgpath + "/{:s}/{:s}_{:06d}.jpg".format(
                    videoname, videoname, frame_num
                )
            ...
            if self.frame_level_list[video_id][frame_num]["labeled"]:
                mask[i] = 1
                all_boxes.append(
                    self.frame_level_list[video_id][frame_num]["boxes"].copy()
                )
                labels.append(
                    self.frame_level_list[video_id][frame_num]["labels"].copy()
                )
                ego_labels.append(
                    self.frame_level_list[video_id][frame_num]["ego_label"]
                )
            else:
        ...
        if self.DATASET == "ava":
            assert (
                keyframe in indexs
            ), " keyframe is not in frame {} from startframe {} and stepsize of {}".format(
                keyframe, start_frame, step_size
            )
    ...
    for c1, bs_ in enumerate(boxes):
        for c2, bs in enumerate(bs_):
            if counts[c1, c2] > 0:
                ...
                assert (
                    targets_temp.shape[0] == bs.shape[0]
                ), "num of labels and boxes should be same"
                new_targets[c1, c2, : counts[c1, c2], :] = torch.from_numpy(
                    targets_temp
                )
