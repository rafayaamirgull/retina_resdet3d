...
def gen_dets(args, net, val_dataset):
    ...
    for epoch in args.EVAL_EPOCHS:
        ...
        args.det_save_dir = os.path.join(
            args.SAVE_ROOT,
            "detections-{it:02d}-{sq:02d}-{n:d}/".format(
                it=epoch, sq=args.TEST_SEQ_LEN, n=int(100 * args.GEN_NMS)
            ),
        )
        ...
        mAP, _, ap_strs = perform_detection(
            args, net, val_data_loader, val_dataset, epoch
        )
...
def perform_detection(args, net, val_data_loader, val_dataset, iteration):
    ...
    with torch.no_grad():
        for val_itr, (
            ...
            if print_time and val_itr % val_step == 0:
                ...
                logger.info(
                    "im_detect: {:d}/{:d} time taken {:0.3f}".format(
    ...
    mAP, ap_all, ap_strs = evaluate.evaluate(
        gt_boxes_all, det_boxes, args.all_classes, iou_thresh=args.IOU_THRESH
    )
    ...
    for vid, videoname in enumerate(val_dataset.video_list):
        ...
        for frame_name in frames_list:
            ...
            if args.JOINT_4M_MARGINALS:
                frame_dets = make_joint_probs_from_marginals(
                    frame_dets, val_dataset.childs, args.num_classes_list
                )
        ...
        if frame_dets.shape[0] > 0:
            ...
            if args.CLASSWISE_NMS:
                cls_dets = utils.filter_detections(
                    args, torch.from_numpy(scores), torch.from_numpy(boxes)
                )
                ...
                cls_dets = np.hstack(
                    (boxes[sorted_ind, :], scores[sorted_ind, np.newaxis])
                )
...
def eval_framewise_dets(args, val_dataset):
    for epoch in args.EVAL_EPOCHS:
        log_file = open(
            "{pt:s}/frame-level-resutls-{it:06d}-{sq:02d}-{n:d}.log".format(
                pt=args.SAVE_ROOT,
                it=epoch,
                sq=args.TEST_SEQ_LEN,
                n=int(100 * args.GEN_NMS),
            ),
            "a",
            10,
        )
        ...
        args.det_file_name = (
            "{pt:s}/frame-level-dets-{it:02d}-{sq:02d}-{n:d}.pkl".format(
                pt=args.SAVE_ROOT,
                it=epoch,
                sq=args.TEST_SEQ_LEN,
                n=int(100 * args.GEN_NMS),
        ...
        result_file = "{pt:s}/frame-ap-results-{it:02d}-{sq:02d}-{n:d}.json".format(
            pt=args.SAVE_ROOT, it=epoch, sq=args.TEST_SEQ_LEN, n=int(100 * args.GEN_NMS)
        )

        if args.JOINT_4M_MARGINALS:
            log_file = open(
                "{pt:s}/frame-level-resutls-{it:06d}-{sq:02d}-{n:d}-j4m.log".format(
                    pt=args.SAVE_ROOT,
                    it=epoch,
                    sq=args.TEST_SEQ_LEN,
                    n=int(100 * args.GEN_NMS),
            ...
            args.det_file_name = (
                "{pt:s}/frame-level-dets-{it:02d}-{sq:02d}-{n:d}-j4m.pkl".format(
                    pt=args.SAVE_ROOT,
                    it=epoch,
                    sq=args.TEST_SEQ_LEN,
                    n=int(100 * args.GEN_NMS),
            ...
            result_file = (
                "{pt:s}/frame-ap-results-{it:02d}-{sq:02d}-{n:d}-j4m.json".format(
                    pt=args.SAVE_ROOT,
                    it=epoch,
                    sq=args.TEST_SEQ_LEN,
                    n=int(100 * args.GEN_NMS),
        ...
        if doeval or not os.path.isfile(result_file):
            ...
            for subset in args.SUBSETS:
                ...
                sresults = evaluate_frames(
                    val_dataset.anno_file,
                    args.det_file_name,
                    subset,
                    iou_thresh=0.5,
